...
exports.connect_chatter = function(config) {
    ...
    var roomConnected = room.server.eio.clientsCount;
    ...
    config.socket.on('chat message', function(data) {
        ...
        if (!data.msg || data.name.length > 15 || data.msg.length > 500) {
            config.socket.emit('alerta', "Hubo un error inesperado, recarga la página, puede ser que el mensaje sea muy largo.");
            return;
        }
        //Emite el mensaje
    ...
    config.socket.on('changeRoom', function(data) {
        ...
        if (owner) {
            if (data.img) {
                ...
                room.img = data.img;
                devolucion.msg = "Se estableció el link enviado a la sala!"
            }
            if (data.pass) {
