...
function emitir(){ //Emito los mensajes nuevos
  ...
  if(s_prv_name && s_prv_id){
    socket.emit('prvt message', {msg: msg.val(), to: s_prv_id, name: configs.name});
  }else{
    socket.emit('chat message', {msg: msg.val(), name: configs.name}); //Emito el msj al servidor
    chat.append($('<div class="media '+msj_class+'">').html('<div class="media-body"><h5 class="media-heading" style="font-weight: bold">Yo: </h5>'+msg.val()+'</div>'));
  }
...
function setData(){ //Guardo los datos del modal de configuracion
  ...
  socket.emit('changeData',{name:final_name});
}
...
function setSala(){ //Hago seteos a la sala en caso de ser el dueño
  if(configs.owner) cambiarBg(sm_img.val());
  if(sm_pass.val() || sm_img.val()) socket.emit('changeRoom', {pass: sm_pass.val(), img: sm_img.val()});
}
